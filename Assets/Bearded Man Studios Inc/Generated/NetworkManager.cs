...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] GameplayControllerNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is GameplayControllerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (GameplayControllerNetworkObject.Length > 0 && GameplayControllerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(GameplayControllerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<GameplayControllerBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateGameplayController instead, its shorter and easier to type out ;)")]
		public GameplayControllerBehavior InstantiateGameplayControllerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(GameplayControllerNetworkObject[index]);
			var netBehavior = go.GetComponent<GameplayControllerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<GameplayControllerBehavior>().networkObject = (GameplayControllerNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of GameplayController
		/// </summary>
		/// <returns>
		/// A local instance of GameplayControllerBehavior
		/// </returns>
		/// <param name="index">The index of the GameplayController prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public GameplayControllerBehavior InstantiateGameplayController(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(GameplayControllerNetworkObject[index]);
			var netBehavior = go.GetComponent<GameplayControllerBehavior>();
